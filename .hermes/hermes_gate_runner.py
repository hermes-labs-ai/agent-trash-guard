"""Self-contained deterministic repository runner; copied by init, patched in this repository.

Upstream `hermes-gate init` copies this runner byte-for-byte. This repository carries a
local patch (see RUNNER_PATCH) so a hosted checkout, which has no worktree changes at all,
compares an explicit review range instead of silently checking nothing.
"""
